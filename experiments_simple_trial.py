...
if __name__ == '__main__':
    ...
    if dataset_name == "traffic_signs":
        network_architecture = models.STNTrafficSignNet
    ...
    experiment_results_folder = os.path.join(configs.results_folder_stn_set1, dataset_name)
    ...
    if not os.path.exists(configs.results_folder_stn_set1):
        os.mkdir(configs.results_folder_stn_set1)
    ...
    tgdp_training_hps_gen = helper.GridSearch(experiment_hps_sets["TPGD_train"])
    tgdp_attack_hps_gen = helper.GridSearch(experiment_hps_sets["TPGD_attack"])
...
def experiment_1_func(net, _loss_fn, _training_dataset, _testing_dataset, epochs, net_name="", train_attack=None,
    ...
    if load_checkpoint:
        ...
        fgsm_hp, fgsm_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                        fgsm_attack_hps_gen, net_hp, attacks_trial.FGSM,
        ...
        pgd_hp, pgd_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgd_attack_hps_gen, net_hp, attacks_trial.PGD,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
        ...
        pgdl2_hp, pgdl2_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgd_attack_hps_gen, net_hp, attacks_trial.PGDl2,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        ...
        tpgd_hp, tpgd_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgd_attack_hps_gen, net_hp, attacks_trial.TPGD,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        ...
        resistance_results = helper.measure_resistance_on_test(net, _loss_fn, _testing_dataset,
                                                               to_attacks=[(attacks_trial.FGSM, fgsm_hp),
                                                                           (attacks_trial.PGD, pgd_hp), 
                                                                           (attacks_trial.PGDl2, pgdl2_hp),
                                                                           (attacks_trial.TPGD, tpgd_hp)],
                                                               device=device,
...
def experiment_2_func(net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
    ...
    if test_FGSM:
        ...
        experiment_1_func(fgsm_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with FGSM adversarial training".format(net_name), train_attack=attacks_trial.FGSM,
    ...
    if test_PGD:
        ...
        experiment_1_func(pgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGD adversarial training".format(net_name), train_attack=attacks_trial.PGD,
    ...
    if test_pgdl2:
        ...
        experiment_1_func(pgdl2_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGDl2 adversarial training".format(net_name), train_attack=attacks_trial.PGDl2,
                          attack_training_hps_gen=pgd_training_hps_gen,
                          load_checkpoint=load_checkpoint, save_checkpoint=save_checkpoint, show_plots=show_plots,
                          save_plots=save_plots,
                          show_validation_accuracy_each_epoch=show_validation_accuracy_each_epoch)
    ...
    if test_tpgd:
        ...
        experiment_1_func(tpgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with TPGD adversarial training".format(net_name), train_attack=attacks_trial.TPGD,
                          attack_training_hps_gen=pgd_training_hps_gen,
                          load_checkpoint=load_checkpoint, save_checkpoint=save_checkpoint, show_plots=show_plots,
                          save_plots=save_plots,
                          show_validation_accuracy_each_epoch=show_validation_accuracy_each_epoch)    
