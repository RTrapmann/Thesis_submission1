...
import datasets_resnet50
import logger
import trainer
import matplotlib
import configs_resnet50_cap

if not configs_resnet50_cap.show_attacks_plots:
    matplotlib.use('Agg')
...
def show_img_lst(imgs, titles=None, x_labels=None, main_title=None, columns=2, plot_img=True, save_img=True,
    ...
    for i in range(len(imgs)):
        fig.add_subplot(int(2 * rows), columns, 2 * i + 1 if i % 2 == 0 else 2 * i)
...
def measure_resistance_on_test(net, loss_fn, test_dataset, to_attacks, device=None, plots_title="", plot_results=True,
                               save_figs=True, figs_path=None):
    """
...
def full_train_of_nn_with_hps(net, loss_fn, train_dataset, hps_gen, epochs, device=None, train_attack=None,
    ...
    if hps_gen.size() > 1 or (hps_gen.size() == 1 and early_stop):
        while True:
            ...
            train_dl, val_dl = datasets_resnet50.get_train_val_dls(train_dataset, hp["batch_size"])
...
def full_attack_of_trained_nn_with_hps(net, loss_fn, train_dataset, hps_gen, selected_nn_hp, attack_method, device=None,
                                       plots_title="", plot_results=True, save_figs=True, figs_path=True):
    """
    ...
    """
    hps_gen.restart()
    train_dl, val_dl = datasets_resnet50.get_train_val_dls(train_dataset, selected_nn_hp["batch_size"])
