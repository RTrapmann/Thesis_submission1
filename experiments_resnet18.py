...
import torchvision
import attacks_new
import configs_resnet18
import datasets_resnet18
import helper_plots
import models_resnet18
import trainer
import os
import shutil
import logger
import argparse
from models_resnet18 import Conv2dAuto, ResidualBlock,ResNetResidualBlock, ResNetBasicBlock, ResNet, ResNetBottleNeckBlock, ResNet18, ResNetLayer, ResNetEncoder, ResnetDecoder
...
net = ResNet18(in_channels = 3, n_classes = 43)
...
if __name__ == '__main__':
    ...
    if dataset_name == "traffic_signs":
        network_architecture = models_resnet18.ResNet


    # load configs_resnet101 from configs_resnet101.py
    experiment_configs = configs_resnet18.configs_dict[dataset_name]["configs"]
    experiment_hps_sets = configs_resnet18.configs_dict[dataset_name]["hps_dict"]
    experiment_results_folder = os.path.join(configs_resnet18.results_folder_resnet18_set4, dataset_name)
    experiment_checkpoints_folder = os.path.join(configs_resnet18.checkpoints_folder, dataset_name)
    ...
    if dataset_name == "traffic_signs":
        assert os.path.exists(configs_resnet18.data_root_dir), "The dataset should be in ./data/GTSRB"
        assert os.path.exists(os.path.join(configs_resnet18.data_root_dir, "GTSRB")), "The dataset should be in ./data/GTSRB"

    if not os.path.exists(configs_resnet18.results_folder_resnet18_set4):
        os.mkdir(configs_resnet18.results_folder_resnet18_set4)
    if not os.path.exists(experiment_results_folder):
        os.mkdir(experiment_results_folder)
    if not os.path.exists(configs_resnet18.checkpoints_folder):
        os.mkdir(configs_resnet18.checkpoints_folder)
    ...
    logger.log_print("save checkpoints: {}".format(configs_resnet18.save_checkpoints))
    logger.log_print("load checkpoints: {}".format(configs_resnet18.load_checkpoints))
    logger.log_print("results folder: {}".format(experiment_results_folder))
    logger.log_print("show results:  {}".format(configs_resnet18.show_attacks_plots))
    logger.log_print("save results:  {}".format(configs_resnet18.save_attacks_plots))

    # seed
    if configs_resnet18.seed is not None:
        torch.manual_seed(configs_resnet18.seed)
        logger.log_print("seed: {}".format(configs_resnet18.seed))
    ...
    if dataset_name == "traffic_signs":
        ...
        _training_dataset = datasets_resnet18.GTSRB(root_dir=configs_resnet18.data_root_dir, train=True, transform=transform)
        _testing_dataset = datasets_resnet18.GTSRB(root_dir=configs_resnet18.data_root_dir, train=False, transform=transform)
...
if __name__ == '__main__' and run_experiment_1:
    ...
    original_net =  ResNet18(in_channels = 3, n_classes = 43).to(device)
    ...
    exp1_res_dict = experiment_1_func(original_net, _loss_fn, _training_dataset, _testing_dataset, epochs,
                                      net_name=net_name,
                                      save_checkpoint=configs_resnet18.save_checkpoints,
                                      load_checkpoint=configs_resnet18.load_checkpoints,
                                      show_plots=configs_resnet18.show_attacks_plots,
                                      save_plots=configs_resnet18.save_attacks_plots,
                                      show_validation_accuracy_each_epoch=configs_resnet18.show_validation_accuracy_each_epoch)
...
if __name__ == '__main__' and run_experiment_2:
    exp2_net =  ResNet18(in_channels = 3, n_classes = 43).to(device)
