...
import torch
import torchvision
import attacks_resnet34
import configs_resnet34
import datasets
import helper_34
import models_resnet34
import trainer
import os
import shutil
import logger
import argparse
from models_resnet34 import Conv2dAuto, ResidualBlock,ResNetResidualBlock, ResNetBasicBlock, ResNet, ResNetBottleNeckBlock, ResNet34, ResNetLayer, ResNetEncoder, ResnetDecoder
...
net = ResNet34(in_channels = 3, n_classes = 43)
...
if __name__ == '__main__':
    ...
    dataset_name = args.dataset_name  # choose traffic_signs
    if dataset_name == "traffic_signs":
        network_architecture = models_resnet34.ResNet


    # load configs_resnet101 from configs_resnet101.py
    experiment_configs = configs_resnet34.configs_dict[dataset_name]["configs"]
    experiment_hps_sets = configs_resnet34.configs_dict[dataset_name]["hps_dict"]
    experiment_results_folder = os.path.join(configs_resnet34.results_folder_resnet34_set4, dataset_name)
    experiment_checkpoints_folder = os.path.join(configs_resnet34.checkpoints_folder, dataset_name)
    ...
    if dataset_name == "traffic_signs":
        assert os.path.exists(configs_resnet34.data_root_dir), "The dataset should be in ./data/GTSRB"
        assert os.path.exists(os.path.join(configs_resnet34.data_root_dir, "GTSRB")), "The dataset should be in ./data/GTSRB"

    if not os.path.exists(configs_resnet34.results_folder_resnet34_set4):
        os.mkdir(configs_resnet34.results_folder_resnet34_set4)
    if not os.path.exists(experiment_results_folder):
        os.mkdir(experiment_results_folder)
    if not os.path.exists(configs_resnet34.checkpoints_folder):
        os.mkdir(configs_resnet34.checkpoints_folder)
    ...
    logger.log_print("save checkpoints: {}".format(configs_resnet34.save_checkpoints))
    logger.log_print("load checkpoints: {}".format(configs_resnet34.load_checkpoints))
    logger.log_print("results folder: {}".format(experiment_results_folder))
    logger.log_print("show results:  {}".format(configs_resnet34.show_attacks_plots))
    logger.log_print("save results:  {}".format(configs_resnet34.save_attacks_plots))

    # seed
    if configs_resnet34.seed is not None:
        torch.manual_seed(configs_resnet34.seed)
        logger.log_print("seed: {}".format(configs_resnet34.seed))
    ...
    if dataset_name == "traffic_signs":
        ...
        _training_dataset = datasets.GTSRB(root_dir=configs_resnet34.data_root_dir, train=True, transform=transform)
        _testing_dataset = datasets.GTSRB(root_dir=configs_resnet34.data_root_dir, train=False, transform=transform)

    # create hyperparameters generators
    net_training_hps_gen = helper_34.GridSearch(experiment_hps_sets["nets_training"])
    fgsm_attack_hps_gen = helper_34.GridSearch(experiment_hps_sets["FGSM_attack"])
    pgd_attack_hps_gen = helper_34.GridSearch(experiment_hps_sets["PGD_attack"])
    fgsm_training_hps_gen = helper_34.GridSearch(experiment_hps_sets["FGSM_train"])
    pgd_training_hps_gen = helper_34.GridSearch(experiment_hps_sets["PGD_train"])
    pgdl2_training_hps_gen = helper_34.GridSearch(experiment_hps_sets["PGDl2_train"])
    pgdl2_attack_hps_gen = helper_34.GridSearch(experiment_hps_sets["PGDl2_attack"])
    tpgd_training_hps_gen = helper_34.GridSearch(experiment_hps_sets["TPGD_train"])
    tpgd_attack_hps_gen = helper_34.GridSearch(experiment_hps_sets["TPGD_attack"])
...
def experiment_1_func(net, _loss_fn, _training_dataset, _testing_dataset, epochs, net_name="", train_attack=None,
    ...
    if load_checkpoint:
        ...
        if train_attack is not None:
            hps_gen = helper_34.concat_hps_gens(net_training_hps_gen, attack_training_hps_gen)

        # apply hyperparameters-search to get a trained network
        net_state_dict, net_hp = helper_34.full_train_of_nn_with_hps(net, _loss_fn, _training_dataset,
        ...
        fgsm_hp, fgsm_score = helper_34.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                        fgsm_attack_hps_gen, net_hp, attacks_resnet34.FGSM,
        ...
        pgd_hp, pgd_score = helper_34.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgd_attack_hps_gen, net_hp, attacks_resnet34.PGD,
                                                                      device=device, plot_results=False,
                                                                      save_figs=False,
                                                                      figs_path=plots_folder)
        logger.log_print("PGD attack selected hyperparams: {}".format(str(pgd_hp)))

        pgdl2_hp, pgdl2_score = helper_34.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgdl2_attack_hps_gen, net_hp, attacks_resnet34.PGDl2,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        logger.log_print("pgdl2 attack selected hyperparams: {}".format(str(pgdl2_hp)))
        
        tpgd_hp, tpgd_score = helper_34.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      tpgd_attack_hps_gen, net_hp, attacks_resnet34.TPGD,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        ...
        resistance_results = helper_34.measure_resistance_on_test(net, _loss_fn, _testing_dataset,
                                                               to_attacks=[(attacks_resnet34.FGSM, fgsm_hp),
                                                                           (attacks_resnet34.PGD, pgd_hp),
                                                                            (attacks_resnet34.PGDl2, pgdl2_hp),
                                                                           (attacks_resnet34.TPGD, tpgd_hp)],
                                                               device=device,
...
if __name__ == '__main__' and run_experiment_1:
    ...
    original_net =  ResNet34(in_channels = 3, n_classes = 43).to(device)
    ...
    exp1_res_dict = experiment_1_func(original_net, _loss_fn, _training_dataset, _testing_dataset, epochs,
                                      net_name=net_name,
                                      save_checkpoint=configs_resnet34.save_checkpoints,
                                      load_checkpoint=configs_resnet34.load_checkpoints,
                                      show_plots=configs_resnet34.show_attacks_plots,
                                      save_plots=configs_resnet34.save_attacks_plots,
                                      show_validation_accuracy_each_epoch=configs_resnet34.show_validation_accuracy_each_epoch)
...
def experiment_2_func(net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
    ...
    """
     Apply adversarial training with FGSM, PGD, PGDL2, TPGD and then analyze it. the parameters are the same as in experiment 1.
    """
    if test_FGSM:
        adversarial_epochs.restart()
        net.apply(helper_34.weight_reset)
        ...
        experiment_1_func(fgsm_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with FGSM adversarial training".format(net_name), train_attack=attacks_resnet34.FGSM,
    ...
    if test_PGD:
        ...
        experiment_1_func(pgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGD adversarial training".format(net_name), train_attack=attacks_resnet34.PGD,
    ...
    if test_pgdl2:
        ...
        experiment_1_func(pgdl2_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGDl2 adversarial training".format(net_name), train_attack=attacks_resnet34.PGDl2,
    ...
    if test_tpgd:
        ...
        experiment_1_func(tpgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with TPGD adversarial training".format(net_name), train_attack=attacks_resnet34.TPGD,
...
if __name__ == '__main__' and run_experiment_2:
    exp2_net =  ResNet34(in_channels = 3, n_classes = 43).to(device)
...
#         cap_net = models_resnet50.create_conv_nn(base_net_params)
