...
import torch
import torchvision
import attacks_resnet50
import configs_resnet50_cap
import datasets
import helper
import models_resnet50_cap
import trainer
import os
import shutil
import logger
import argparse
from models_resnet50_cap import Bottleneck, ResNet, ResNet50
...
if __name__ == '__main__':
    ...
    if dataset_name == "traffic_signs":
        network_architecture = models_resnet50_cap.ResNet


    # load configs_resnet50 from configs_resnet50.py
    experiment_configs = configs_resnet50_cap.configs_dict[dataset_name]["configs"]
    experiment_hps_sets = configs_resnet50_cap.configs_dict[dataset_name]["hps_dict"]
    experiment_results_folder = os.path.join(configs_resnet50_cap.results_folder_resnet50_set4, dataset_name)
    experiment_checkpoints_folder = os.path.join(configs_resnet50_cap.checkpoints_folder, dataset_name)
    ...
    if dataset_name == "traffic_signs":
        assert os.path.exists(configs_resnet50_cap.data_root_dir), "The dataset should be in ./data/GTSRB"
        assert os.path.exists(os.path.join(configs_resnet50_cap.data_root_dir, "GTSRB")), "The dataset should be in ./data/GTSRB"

    if not os.path.exists(configs_resnet50_cap.results_folder_resnet50_set4):
        os.mkdir(configs_resnet50_cap.results_folder_resnet50_set4)
    if not os.path.exists(experiment_results_folder):
        os.mkdir(experiment_results_folder)
    if not os.path.exists(configs_resnet50_cap.checkpoints_folder):
        os.mkdir(configs_resnet50_cap.checkpoints_folder)
    ...
    logger.log_print("save checkpoints: {}".format(configs_resnet50_cap.save_checkpoints))
    logger.log_print("load checkpoints: {}".format(configs_resnet50_cap.load_checkpoints))
    logger.log_print("results folder: {}".format(experiment_results_folder))
    logger.log_print("show results:  {}".format(configs_resnet50_cap.show_attacks_plots))
    logger.log_print("save results:  {}".format(configs_resnet50_cap.save_attacks_plots))

    # seed
    if configs_resnet50_cap.seed is not None:
        torch.manual_seed(configs_resnet50_cap.seed)
        logger.log_print("seed: {}".format(configs_resnet50_cap.seed))
    ...
    if dataset_name == "traffic_signs":
        ...
        _training_dataset = datasets.GTSRB(root_dir=configs_resnet50_cap.data_root_dir, train=True, transform=transform)
        _testing_dataset = datasets.GTSRB(root_dir=configs_resnet50_cap.data_root_dir, train=False, transform=transform)
    ...
    pgdl2_training_hps_gen = helper.GridSearch(experiment_hps_sets["PGDl2_train"])
    pgdl2_attack_hps_gen = helper.GridSearch(experiment_hps_sets["PGDl2_attack"])
    tpgd_training_hps_gen = helper.GridSearch(experiment_hps_sets["TPGD_train"])
    tpgd_attack_hps_gen = helper.GridSearch(experiment_hps_sets["TPGD_attack"])
...
def experiment_1_func(net, _loss_fn, _training_dataset, _testing_dataset, epochs, net_name="", train_attack=None,
    ...
    if load_checkpoint:
        ...
        fgsm_hp, fgsm_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                        fgsm_attack_hps_gen, net_hp, attacks_resnet50.FGSM,
        ...
        pgd_hp, pgd_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgd_attack_hps_gen, net_hp, attacks_resnet50.PGD,
                                                                      device=device, plot_results=False,
                                                                      save_figs=False,
                                                                      figs_path=plots_folder)
        logger.log_print("PGD attack selected hyperparams: {}".format(str(pgd_hp)))

        pgdl2_hp, pgdl2_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      pgdl2_attack_hps_gen, net_hp, attacks_resnet50.PGDl2,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        logger.log_print("pgdl2 attack selected hyperparams: {}".format(str(pgdl2_hp)))
        
        tpgd_hp, tpgd_score = helper.full_attack_of_trained_nn_with_hps(net, _loss_fn, _training_dataset,
                                                                      tpgd_attack_hps_gen, net_hp, attacks_resnet50.TPGD,
                                                                      device=device, plot_results=True,
                                                                      save_figs=True,
                                                                      figs_path=plots_folder)
        ...
        resistance_results = helper.measure_resistance_on_test(net, _loss_fn, _testing_dataset,
                                                               to_attacks=[(attacks_resnet50.FGSM, fgsm_hp),
                                                                           (attacks_resnet50.PGD, pgd_hp),
                                                                            (attacks_resnet50.PGDl2, pgdl2_hp),
                                                                           (attacks_resnet50.TPGD, tpgd_hp)],
                                                               device=device,
...
if __name__ == '__main__' and run_experiment_1:
    ...
    original_net = ResNet50(43).to(device)
    ...
    exp1_res_dict = experiment_1_func(original_net, _loss_fn, _training_dataset, _testing_dataset, epochs,
                                      net_name=net_name,
                                      save_checkpoint=configs_resnet50_cap.save_checkpoints,
                                      load_checkpoint=configs_resnet50_cap.load_checkpoints,
                                      show_plots=configs_resnet50_cap.show_attacks_plots,
                                      save_plots=configs_resnet50_cap.save_attacks_plots,
                                      show_validation_accuracy_each_epoch=configs_resnet50_cap.show_validation_accuracy_each_epoch)
...
def experiment_2_func(net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
    ...
    if test_FGSM:
        ...
        experiment_1_func(fgsm_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with FGSM adversarial training".format(net_name), train_attack=attacks_resnet50.FGSM,
    ...
    if test_PGD:
        ...
        experiment_1_func(pgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGD adversarial training".format(net_name), train_attack=attacks_resnet50.PGD,
    ...
    if test_pgdl2:
        ...
        experiment_1_func(pgdl2_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with PGDl2 adversarial training".format(net_name), train_attack=attacks_resnet50.PGDl2,
    ...
    if test_tpgd:
        ...
        experiment_1_func(tpgd_robust_net, _loss_fn, _training_dataset, _testing_dataset, adversarial_epochs,
                          net_name="{} with TPGD adversarial training".format(net_name), train_attack=attacks_resnet50.TPGD,
...
if __name__ == '__main__' and run_experiment_2:
    exp2_net = ResNet50(43).to(device)
...
#     net = models_resnet50_cap.Resnet101TrafficSign.to(device)
