    ...
    def test_attack(self, dataloader, plot_results=True, save_results_figs=True, fig_path=None, main_title="",
        ...
        for batch_num, (xs, ys) in enumerate(dataloader):
            ...
            if plot_results or save_results_figs:
                ...
                for idx in range(len(batch_successful_attack)):
                    if len(successful_attacks_details) < configs_resnet34.imgs_to_show and \
        ...
        if plot_results or save_results_figs:
            ...
            if len(to_plot_imgs) > 0:
                helper_34.show_img_lst(to_plot_imgs, to_plot_titles, to_plot_xlabels, main_title, columns=2,
...
class PGDl2(Attack):
    ...
    def perturb(self, X, y, device=None):
        """ generates adversarial examples to given data points and labels (X, y) based on TPGD approach. """
        """"    [https://arxiv.org/abs/1901.08573]
"""
        original_X = X
        batch_size = len(original_X)
        for i in range(self.steps):
            X.requires_grad_()
            outputs = self.net(X)
            ...
            diff = X - original_X
            diff_norms = torch.norm(diff.view(batch_size, -1), p=2, dim=1)
            factor = self.epsilon / diff_norms 
            factor = torch.min(factor, torch.ones_like(diff_norms))
...
class TPGD(Attack):
    ...
    def perturb(self, X, y, device=None):
        ...
        X = 0.001 * torch.randn_like(original_X)
